    ...
    if (!ParentModel) {
      ...
      ParentModel.findOne({ id: req.body[parentIdProperty] })
        ...
        .exec(function (err, parent) {
          ...
          if (!parent)
            return res.error({
              message: `${Helper.capitalizeFirstLetter(parentName)} not found`,
            });
          ...
          if (!parent[`${req.body.subject}s`])
            return res.internal({
              message: `${Helper.capitalizeFirstLetter(parentName)} ${
                req.body.subject
              }s not found`,
            });
...
const UpdateAction = (req, res) => {
  ...
  Model.findOne({ id: req.body.id }).exec(function (err, target) {
    ...
    if (!target)
      return res.error({
        message: `${Helper.capitalizeFirstLetter(req.body.subject)} not found`,
      });
...
const DeleteAction = (req, res) => {
  ...
  Model.findOne({ id: req.body.id }).exec(function (err, model) {
    ...
    if (!model)
      return res.error({
        message: `${Helper.capitalizeFirstLetter(req.body.type)} not found`,
      });
