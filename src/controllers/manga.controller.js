...
const GetManga = (req, res) => {
    if (!req.query.mangaId)
        return res.error({ message: "Manga id is required" });

    let find;
    if (req.query.isOnly === "true") {
        find = Manga.findOne({ id: req.query.mangaId }).select(
...
const GetMangaUpdate = (req, res) => {
    ...
    Manga.findOne({ id: req.query.mangaId })
        ...
        .exec((err, manga) => {
            if (err)
                return res.error({ message: "Get manga failed", errors: err });
            if (!manga) return res.error({ message: "Manga not found" });
...
const GetSection = (req, res) => {
    ...
    Section.findOne({ id: req.query.sectionId }).exec((err, section) => {
        if (err)
            return res.error({ message: "Get section failed", errors: err });
    ...
    SystemStatus.findOne({}).exec(function (err, SystemStatus) {
        ...
        JSZip.loadAsync(req.file.buffer).then(function (zip) {
                    ...
                    return await bucket
                        ...
                        .then(() => {
                            listPages.push({
                                ...
                                pageUrl:
                                    process.env.GCP_STORAGE_URL + params.Path,
                            });
                        });
                });
            ...
            Promise.all(listPromises).then(() => {
                ...
                // update manga
                Manga.findOne({ id: req.body.mangaId }).exec(function (
                    err,
                    manga
                ) {
                    if (err)
                    ...
                    if (!manga)
                        return res.error({ message: `Manga not found` });
                    // update section
    ...
    Chapter.findOne({ id: req.body.id }).exec(async function (err, chapter) {
        ...
        if (req.file) {
            if (path.extname(req.file?.originalname) !== ".zip") {
                return res
                    .status(400)
                    .json({ error: "File must be a zip file" });
            }
